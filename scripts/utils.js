...
const knoxDir = path.join(__dirname, "..", "..", "..");

function copyConfigSchema() {
  // Modify and copy for .knoxrc.json
  const schema = JSON.parse(fs.readFileSync("config_schema.json", "utf8"));
  schema.$defs.SerializedBuddyConfig.properties.mergeBehavior = {
    type: "string",
    enum: ["merge", "overwrite"],
    default: "merge",
    title: "Merge behavior",
    markdownDescription:
      "If set to 'merge', .knoxrc.json will be applied on top of config.json (arrays and objects are merged). If set to 'overwrite', then every top-level property of .knoxrc.json will overwrite that property from config.json.",
  };
  fs.writeFileSync("knox_rc_schema.json", JSON.stringify(schema, null, 2));
}
...
function installNodeModules() {
  ...
  if (!process.cwd().endsWith("vscode")) {
    process.chdir(path.join(knoxDir, "extensions", "vscode"));
  }
  ...
  process.chdir(path.join(knoxDir, "gui"));
...
async function downloadEsbuildBinary(target) {
  ...
  const downloadUrl = {
    ...
    "linux-arm64":
      "https://registry.npmjs.org/@esbuild/linux-arm64/-/linux-arm64-0.25.10.tgz",
    "win32-arm64":
      "https://registry.npmjs.org/@esbuild/win32-arm64/-/win32-arm64-0.25.10.tgz",
    "linux-x64":
      "https://registry.npmjs.org/@esbuild/linux-x64/-/linux-x64-0.25.10.tgz",
    "darwin-x64":
      "https://registry.npmjs.org/@esbuild/darwin-x64/-/darwin-x64-0.25.10.tgz",
    "win32-x64":
      "https://registry.npmjs.org/@esbuild/win32-x64/-/win32-x64-0.25.10.tgz",
  }[target];
...
async function downloadSqliteBinary(target) {
  console.log("[info] Downloading pre-built sqlite3 binary for target:", target);
  ...
  if (!downloadUrl) {
    throw new Error(`No pre-built sqlite3 binary available for target: ${target}`);
  }
...
async function rebuildSqliteBinary(target) {
  console.log("[info] Rebuilding sqlite3 binary for target:", target);
  ...
  try {
    ...
    // Rebuild sqlite3 for current architecture
    execCmdSync("npm rebuild sqlite3");
    console.log("[info] Successfully rebuilt sqlite3 for current architecture");
  } finally {
    process.chdir(originalCwd);
  }
}

async function ensureSqliteBinaryCorrectArch(target) {
  console.log("[info] Ensuring sqlite3 binary matches target architecture:", target);
  const sqliteBinaryPath = path.join(__dirname, "../../../core/node_modules/sqlite3/build/Release/node_sqlite3.node");
  
  // Check if sqlite3 binary exists
  if (!fs.existsSync(sqliteBinaryPath)) {
    console.log("[info] SQLite3 binary not found, rebuilding...");
    await rebuildSqliteBinary(target);
    return;
  }
  
  // Check binary architecture (only on macOS/Linux where 'file' command is available)
  if (process.platform === "darwin" || process.platform === "linux") {
    try {
      const { execSync } = require("child_process");
      const fileOutput = execSync(`file "${sqliteBinaryPath}"`, { encoding: 'utf8' });
      console.log("[info] Current sqlite3 binary architecture:", fileOutput.trim());
      
      // Check if architecture matches
      const expectedArch = target.includes("arm64") ? "arm64" : "x86_64";
      const hasCorrectArch = fileOutput.includes(expectedArch);
      
      if (!hasCorrectArch) {
        console.log(`[info] SQLite3 binary architecture mismatch. Expected: ${expectedArch}, rebuilding...`);
        await rebuildSqliteBinary(target);
      } else {
        console.log("[info] SQLite3 binary architecture is correct");
      }
    } catch (error) {
      console.warn("[warn] Could not verify sqlite3 binary architecture, rebuilding to be safe:", error.message);
      await rebuildSqliteBinary(target);
    }
  } else {
    // On Windows, just rebuild to be safe
    console.log("[info] On Windows, rebuilding sqlite3 to ensure correct architecture");
    await rebuildSqliteBinary(target);
  }
}
...
async function installNodeModuleInTempDirAndCopyToCurrent(packageName, toCopy) {
  ...
  const tempDir = `/tmp/knox-node_modules-${adjustedName}`;
