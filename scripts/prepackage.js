...
(async () => {
  ...
  // Ensure sqlite3 binary is built for the correct architecture
  const { ensureSqliteBinaryCorrectArch } = require("./utils");
  await ensureSqliteBinaryCorrectArch(target);
  ...
  if (isInGitHubAction && isArmTarget) {
    ...
    const downloadUrl = {
      ...
      "win32-arm64":
        "https://knox-server-binaries.s3.us-west-1.amazonaws.com/win32-arm64/node_sqlite3.tar.gz",
    }[target];
    execCmdSync(
    ...
    execCmdSync(
      `curl -o node_modules/@esbuild/esbuild.zip https://knox-server-binaries.s3.us-west-1.amazonaws.com/${target}/esbuild.zip`,
    );
  ...
  // Copy checkpoint native module
  const checkpointModulePath = path.join(__dirname, "../../../core/checkpoints/index.node");
  if (fs.existsSync(checkpointModulePath)) {
    fs.cpSync(checkpointModulePath, "out/checkpoints.node");
    console.log("[info] Copied checkpoint native module");
  } else {
    console.warn("[warn] Checkpoint native module not found, skipping");
  }
