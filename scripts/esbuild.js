...
// Function to copy checkpoint native module
function copyCheckpointModule() {
  const sourcePath = path.resolve(__dirname, "../../../core/checkpoints/index.node");
  const destPath = path.resolve(__dirname, "../out/checkpoints.node");
  
  try {
    if (fs.existsSync(sourcePath)) {
      fs.copyFileSync(sourcePath, destPath);
      console.log("✅ Copied checkpoint native module to out/checkpoints.node");
    } else {
      console.warn("⚠️ Checkpoint native module not found. Run 'npm run build' in core/checkpoints/");
    }
  } catch (error) {
    console.warn("⚠️ Failed to copy checkpoint module:", error.message);
  }
}
